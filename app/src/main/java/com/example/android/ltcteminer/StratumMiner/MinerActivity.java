...
import com.example.android.ltcteminer.MainActivity;
import com.example.android.ltcteminer.R;
import com.example.android.ltcteminer.StratumMiner.MinerService;
import com.example.android.ltcteminer.StratumMiner.Worker.CpuMiningWorker;

import java.text.DecimalFormat;

import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_BACKGROUND;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_PASS;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_PRIORITY;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_SCREEN;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_THREAD;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_URL;
import static com.example.android.ltcteminer.StratumMiner.Constants.DEFAULT_USER;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_BACKGROUND;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_PASS;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_PRIORITY;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_SCREEN;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_THREAD;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_TITLE;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_URL;
import static com.example.android.ltcteminer.StratumMiner.Constants.PREF_USER;
...
public class MinerActivity extends AppCompatActivity {
    ...
    TextView url_view;
    TextView user_View;
    TextView threads_view;
    TextView priority_view;
    TextView runback_view;
    TextView keepscreen_view;
    Button b_close_miner;
...
//                firstRunFlag = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toolbar myToolbar = (Toolbar) findViewById(R.id.my_toolbar_miner);
        myToolbar.setLogo(R.mipmap.ltcte_actionbar_icon);
        myToolbar.setTitle(R.string.miner_activity_actionbar_title);
        ...
        if (bundle != null) {
            mServer = bundle.getString("server",DEFAULT_URL);
            mUser = bundle.getString("user",DEFAULT_USER);
            mPassword= bundle.getString("password",DEFAULT_PASS);
            mThreads = bundle.getInt("threads_num",DEFAULT_THREAD);
            mPriority = bundle.getInt("threads_priority",DEFAULT_PRIORITY);
            isBackground = bundle.getBoolean("run_background",false);
            isScreen = bundle.getBoolean("screen_awake",false);
        } else {
            mServer = DEFAULT_URL;
            mUser = DEFAULT_USER;
            mPassword = DEFAULT_PASS;
            mThreads = DEFAULT_THREAD;
...
//        setTitle("StratumMiner");
        ...
        btn_startStop = (Button) findViewById(R.id.status_button_startstop);
        b_close_miner = (Button) findViewById(R.id.miner_activity_closeminer);
        url_view = (TextView) findViewById(R.id.url_text_view);
        url_view.setText(mServer);
        user_View = (TextView) findViewById(R.id.user_text_view);
        user_View.setText(mUser);
        threads_view = (TextView) findViewById(R.id.thread_text_view);
        threads_view.setText("" + mThreads);
        priority_view = (TextView) findViewById(R.id.priority_text_view);
        priority_view.setText("" + mPriority);
        runback_view = (TextView) findViewById(R.id.runback_text_view);
        if (isBackground) {
            runback_view.setText(getString(R.string.activity_miner_on));
        } else {
            runback_view.setText(getString(R.string.activity_miner_off));
        }
        keepscreen_view = (TextView) findViewById(R.id.screenawake_text_view);
        if (isScreen) {
            keepscreen_view.setText(getString(R.string.activity_miner_on));
        } else {
            keepscreen_view.setText(getString(R.string.activity_miner_off));
        }


        b_close_miner.setOnClickListener(new View.OnClickListener() {
            @Override
...
//                    int threads = Integer.parseInt(threadList.getSelectedItem().toString());
                    ...
                    editor.putString(PREF_URL, mServer);
                    editor.putString(PREF_USER, mUser);
                    editor.putString(PREF_PASS, mPassword);
                    editor.putInt(PREF_THREAD, mThreads);
                    editor.putInt(PREF_PRIORITY, mPriority);
                    editor.putBoolean(PREF_BACKGROUND, isBackground);
                    editor.putBoolean(PREF_SCREEN, isScreen);
                    editor.commit();
                    ...
                    firstRunFlag = false;
                    b.setText(getString(R.string.main_button_stop));
                    b_close_miner.setEnabled(false);
                    b_close_miner.setClickable(false);
                }
                else{
                    stopMining();
                    StartShutdown = true;
                    b.setText(getString(R.string.status_button_start));
                    b_close_miner.setEnabled(true);
                    b_close_miner.setClickable(true);
//                                                     b.setEnabled(false);
//                                                     b.setClickable(false);
    ...
    public class ThreadStatusAsyncTask extends AsyncTask<CpuMiningWorker,Integer,Boolean> {
        ...
        protected void onPostExecute(Boolean aBoolean) {
            if (aBoolean) {
                ...
                StartShutdown = false;
                firstRunFlag = true;
                Toast.makeText(MinerActivity.this,"Cooldown finished",Toast.LENGTH_SHORT).show();
            }
        }
    }
